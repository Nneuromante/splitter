...
with tab2:
    ...
    if not st.session_state.scene_gallery:
        ...
        cols_per_row = 3
        rows = [filtered_scenes[i:i + cols_per_row] for i in range(0, len(filtered_scenes), cols_per_row)]
        ...
        for row in rows:
            ...
            for i, scene in enumerate(row):
                with cols[i]:
                    # Display thumbnail
                    if scene["format"] == "gif":
                        st.image(scene["thumbnail"], caption=f"{scene['source_video']} - Scene {scene['scene_number']}")
                    else:
                        st.image(scene["thumbnail"], caption=f"{scene['source_video']} - Scene {scene['scene_number']}")
                    
                    # Display duration
                    st.caption(f"Duration: {scene['duration']:.2f}s")
                    
                    # Create a download button for this scene
