...
with tab1:
    ...
    if process_button:
                    ...
                    for idx, (start, end) in enumerate(scenes, start=1):
                        ...
                        try:
                            ...
                            if os.path.exists(output_file) and os.path.getsize(output_file) > 10000:
                                ...
                                # Debug info
                                st.write(f"Added scene {idx} to gallery (current count: {len(st.session_state.scene_gallery)})")
        ...
        if all_scene_files:
            ...
            # Debug: show final gallery state before switching tab
            st.write(f"Final gallery count: {len(st.session_state.scene_gallery)} scenes")
...
with tab2:
    ...
    # Debug: show gallery state in the gallery tab
    st.write(f"Debug: Gallery contains {len(st.session_state.scene_gallery)} scenes")
    ...
    if not st.session_state.scene_gallery:
        ...
        # Debug: show filtered scenes
        st.write(f"Filtered to {len(filtered_scenes)} scenes")
            ...
            with zipfile.ZipFile(zip_buffer, "w") as zip_file:
                for scene_idx in st.session_state.selected_scenes:
                    try:
                        ...
                        st.error(f"Error accessing scene at index {scene_idx}")
                    except Exception as e:
                        st.error(f"Error adding scene to zip: {str(e)}")
            ...
            with col2:
                ...
                with st.expander("Preview Scene"):
                    try:
                        ...
                        st.error(f"Error displaying scene preview: {str(e)}")
